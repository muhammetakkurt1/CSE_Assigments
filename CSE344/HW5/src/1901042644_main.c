...
int main(int argc, char *argv[]) {
    ...
    pthread_barrier_init(&barrier, NULL, workers_num + 1);  /*Initialize barrier including manager thread*/
...
void release_resources() {
    ...
    pthread_barrier_destroy(&barrier);              /*Destroys the barrier used to synchronize threads during the end of processing*/
}
...
void *worker_thread(void *arg) {            /* Worker thread function for processing files from the buffer*/
    ...
    pthread_barrier_wait(&barrier);         /*Wait at a barrier until all threads have reached this point to ensure synchronized termination*/
    return NULL;
}
...
void *manager_thread(void *arg) {                       /*Manager thread function to populate the buffer with file descriptors*/
    ...
    pthread_mutex_unlock(&buffer_mutex);
    /*Wait at the barrier for all threads (workers and this manager thread) to synchronize before terminating*/
